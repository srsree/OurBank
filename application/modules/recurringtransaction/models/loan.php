...
// 		die($select->__toString($select));
	...
	public function fetchAccountDetails($accountNumber) {
		$select = $this->select()
			...
			->join(array('F' => 'ourbank_productsofferdetails'),'E.product_id=F.offerproduct_id')
			->where('F.recordstatus_id = 3 or F.recordstatus_id = 1')
			->join(array('H' => 'ourbank_productsloan'),'F.offerproductupdate_id=H.offerproductupdate_id')
			->join(array('P' => 'ourbank_productdetails'),'F.product_id=P.product_id')
			->where('P.recordstatus_id = 3 or P.recordstatus_id = 1')
			->join(array('G' => 'ourbank_categorydetails'),'P.category_id=G.category_id')
			->where('G.recordstatus_id = 3 or G.recordstatus_id = 1');
...
// 		die($select->__toString($select));
	...
	public function fetchGroupAccountDetails($accountNumber) {
		$select = $this->select()
			...
			->join(array('F' => 'ourbank_productsofferdetails'),'E.product_id=F.offerproduct_id')
			->where('F.recordstatus_id = 3 or F.recordstatus_id = 1')
			->join(array('Z' => 'ourbank_productsloan'),'F.offerproductupdate_id=Z.offerproductupdate_id')
			->join(array('P' => 'ourbank_productdetails'),'F.product_id=P.product_id')
			->where('P.recordstatus_id = 3 or P.recordstatus_id = 1')
			->join(array('G' => 'ourbank_categorydetails'),'P.category_id=G.category_id')
			->where('G.recordstatus_id = 3 or G.recordstatus_id = 1');
...
// 		die($select->__toString($select));
	...
	public function loanInstallmentbreakupsInsertion($input = array()) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$result = $this->db->insert('ourbank_installmentbreakupsdetails',$input);
		return $result;
	}
	...
	public function totalinstallmentpaid($accountid) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$sql = "SELECT COUNT(accountinstallment_id) installmentpaid,accountinstallment_id FROM ourbank_installmentdetails where account_id=$accountid AND installment_status=2";
		$result = $this->db->fetchAll($sql);
		return $result;
	}
	...
	public function updateinstallment($accountid,$input = array()) {
		...
		$result = $this->db->update('ourbank_installmentbreakupsdetails',$input,$where);
	}
	...
	public function updateinstallments($accountid,$input = array()) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$where[] = "account_id = '".$accountid."'";
		$result = $this->db->update('ourbank_installmentdetails',$input,$where);
		$result = $this->db->update('ourbank_installmentbreakupsdetails',$input,$where);
...
//		die($select->__toString($select));
	...
	public function loanInstalmentscounts($accountId,$i) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$sql = "select SUM(accountinstallment_amount) as totalinstallmentamount,SUM(accountinstallment_interest_amount) as totalinstallmentamountinterest,SUM(installment_principal_amount) as totalinstallmentamountprinciple,accountinstallment_id,accountinstallment_date,statusdescription from   ourbank_installmentdetails A,ourbank_installmentstatus B where A.account_id=$accountId and A.accountinstallment_id=$i and (A.recordstatus_id = 3 OR A.recordstatus_id =1) and (A.installment_status=B.installmentstatus_id)";
	...
	public function selectinstallmentbreakups($accountid,$installmentstatus) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$sql = "SELECT accountinstallment_interest_amount,installment_principal_amount FROM ourbank_installmentbreakupsdetails where (account_id=$accountid AND installment_status=$installmentstatus AND (recordstatus_id=3 or recordstatus_id=1))";
		$result = $this->db->fetchAll($sql);
		return $result;
	}
	...
	public function loanRepaymentInsert($input = array(),$installmentstatus) {
		...
		$result2 = $this->db->update('ourbank_installmentbreakupsdetails',$input2,$where);
		return $result;
	}
	...
	public function nextdueinsert($account_id,$Installmentserial_id) {
		...
		$result1 = $this->db->update('ourbank_installmentdetails',$input3,$where1);
		$result2 = $this->db->update('ourbank_installmentbreakupsdetails',$input3,$where1);
		return $result;
	}

	public function nextdue($accountid) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$sql = "SELECT * FROM ourbank_installmentdetails where (account_id=$accountid AND (recordstatus_id=3 or recordstatus_id=1) AND installment_status != 2) LIMIT 1";
		$result = $this->db->fetchAll($sql);
		return $result;
	}
