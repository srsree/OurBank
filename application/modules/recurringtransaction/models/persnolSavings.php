...
<?php
class Recurringtransaction_Model_persnolSavings extends Zend_Db_Table {
...
// 		die($select->__toString($select));
	...
	public function transferaccountid($accountnumber) {
		$select = $this->select()
			...
			->where('b.member_status = 1 or b.member_status = 3')
			->join(array('c' => 'ourbank_productsofferdetails'),'a.product_id=c.offerproduct_id')
...
// 	}
	...
	public function selectbankassetsaccount($branchID) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$sql = "select * from   ourbank_glsubcode where substr(header,5)='$branchID' and glcode_id='1'";
		$result = $this->db->fetchAll($sql,array());
		return $result;
	}
	...
	public function selectbankcashassetsaccount($branchID) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$sql = "select * from   ourbank_glsubcode where substr(header,5)='$branchID' and glcode_id='2'";
		$result = $this->db->fetchAll($sql,array());
		return $result;
	}
}
