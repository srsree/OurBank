...
class Transaction_Model_recurringSavings extends Zend_Db_Table {
	...
	public function installmentsDetails($accountId,$productId) {
        ...
        $sql = "select * from ourbank_recurringpaydetails A,ourbank_product_fixedrecurring B,ourbank_productsofferdetails C,ourbank_installmentstatus D
		where (A.account_id ='$accountId') AND (C.offerproduct_id='$productId') AND (A.rec_payment_status=D.installmentstatus_id) AND (C.offerproductupdate_id=B.offerproductupdate_id) AND (B.productstatus_id=1 or B.productstatus_id=3) AND (C.recordstatus_id=1 or C.recordstatus_id=3)";
	...
	public function recurringAccountDetails($accountId,$productId) {
		$this->db = Zend_Db_Table::getDefaultAdapter();
		$sql = "select * from ourbank_recurringaccounts A,ourbank_productsofferdetails B,ourbank_product_fixedrecurring C,ourbank_productdetails D,ourbank_accounts E where (A.account_id ='$accountId') AND (B.offerproduct_id='$productId') AND (C.offerproductupdate_id=B.offerproductupdate_id) AND (C.productstatus_id=3) AND (B.recordstatus_id=3) AND (A.recordstatus_id=1) AND (D.product_id=B.product_id) AND (A.account_id=E.account_id)";
		$result = $this->db->fetchAll($sql,array($accountId));
		return $result;
	}
