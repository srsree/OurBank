...
class Reports_Model_Loandemand extends Zend_Db_Table
{
...
->where('e.installment_status=5')
->join(array('f'=>'ourbank_installmentdetails'),'d.account_id = f.account_id',array('SUM(f.accountinstallment_amount) as currentamount','SUM(f.accountinstallment_interest_amount) as currentinterest'))
->where('f.installment_status=4')
->join(array('g'=>'ourbank_installmentdetails'),'d.account_id = g.account_id',array('SUM(g.accountinstallment_amount) as paidamount','SUM(g.accountinstallment_interest_amount) as paidinterest'))
->where('g.installment_status=2')
->join(array('h'=>'ourbank_installmentdetails'),'d.account_id = h.account_id',array('SUM(h.accountinstallment_amount) as nextamount','SUM(h.accountinstallment_interest_amount) as nextinterest'))
->where('h.installment_status=4');
